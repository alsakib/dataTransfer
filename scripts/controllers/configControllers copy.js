...
function fetchData($rootScope, $http) {
	...
	var promise = $http.get(apiUrlConfig).then(function (response) {
		...
		$rootScope.targetServerr['pwd'] = CryptoJS.AES.decrypt(response.data.targetServer.pwd, "5AV<wpN$|9p_}?Sz");
		console.log('CryptoJS_d', CryptoJS.AES.decrypt(response.data.targetServer.pwd, "5AV<wpN$|9p_}?Sz").toString(CryptoJS.enc.Utf8));
	});

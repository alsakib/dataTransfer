...
app.controller('syncController', function ($rootScope, $http) {
	...
	$rootScope.testConnexion = function () {
		...
		//var pwwE = CryptoJS.AES.encrypt("Message", "5AV<wpN$|9p_}?Sz");
		//var pwwD = CryptoJS.AES.decrypt($("#targetServerPwd").val(), "5AV<wpN$|9p_}?Sz");
	...
	$rootScope.process = function () {
		...
		$rootScope.spanClass = 'default';
		$rootScope.processStatut = 'Data processing...';
		//selectedItemGet();
...
function getData($rootScope, $http, ouID, UIDs, periods, level) {
	var requestURL = '../../analytics/dataValueSet.json?dimension=dx:' + UIDs;
...
function pushData($rootScope, $http, _data) {
	...
	$http.post(url, data, { headers: headers }).then(function (response) {
		//console.log('pushData Statut: ', url, data, auth,{ headers: headers })
		...
		$rootScope.processStatut = 'No data Sent ! ';
	})
